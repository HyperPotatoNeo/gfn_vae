...
for epoch in range(args.n_epochs):
    ...
    for i, (x, _) in enumerate(dataloader):
        ...
        # pb
        z_1_dist, z_2_dist, z_1, z_2 = vae.posterior(x, detach_z=True)
        log_pb = z_1_dist.log_prob(z_1.detach()).sum(1) + z_2_dist.log_prob(z_2.detach()).sum(1)
        # pf
        with torch.no_grad():
            p_z_2_dist = torch.distributions.Normal(torch.zeros_like(z_2), torch.ones_like(z_2))
            p_z_1_params =  vae.decoder.z_layers(z_2)
            p_z_1_dist = torch.distributions.Normal(p_z_1_params[: , :args.z_dim], torch.exp(0.5*p_z_1_params[:, args.z_dim:]))
            logits = vae.decoder.decoder(z_1)
            x_dist = torch.distributions.Bernoulli(logits=logits)
            log_pf = p_z_1_dist.log_prob(z_1).sum(1) + p_z_2_dist.log_prob(z_2).sum(1) + x_dist.log_prob(torch.bernoulli(x)).sum(dim=(1, 2, 3))
