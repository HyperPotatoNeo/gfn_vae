...
for epoch in range(args.n_epochs):
    ...
    if epoch % args.log_image_freq == 0 and args.track:
        ...
        for i, ax in enumerate(axs3.flatten()):
            ...
            ax.axis('off')
        wandb.log({"elbo": elbo_epoch, "kl": kl_epoch, "recon_prob": recon_epoch, "elbo_test": elbo_epoch_test, "kl_test": kl_epoch_test, "recon_test": recon_epoch_test, "gen_samples": wandb.Image(fig1), "ground_truth": wandb.Image(fig2), "recon_samples": wandb.Image(fig3)})
        plt.close(fig1)
        plt.close(fig2)
        plt.close(fig3)
    elif args.track:
